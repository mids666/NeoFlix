...
export default function Layout({ children }: { children: React.ReactNode }) {
  return (
    <div className="min-h-screen bg-background text-foreground transition-colors duration-300 selection:bg-red-600 selection:text-white">
      <Navbar />
      <AuthModal />
      <main className="pt-0">
        {children}
        ...
        <div className="max-w-7xl mx-auto grid grid-cols-2 md:grid-cols-4 gap-8">
          ...
          <div className="space-y-4">
            ...
            <ul className="space-y-2 text-muted-foreground text-sm">
              <li><Link to="/browse/movie" className="hover:text-red-600 transition-colors">Movies</Link></li>
              <li><Link to="/browse/tv" className="hover:text-red-600 transition-colors">TV Shows</Link></li>
